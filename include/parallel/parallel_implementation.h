                                     ...
                                     const unsigned int root_id) const
{
  ...
  if (root_id == this->rank())
    {
      for (std::size_t i=0; i<data.size(); ++i)
        bufsize += data[i].size() + 1;  // Add one for the string length byte
    }
  this->broadcast(bufsize, root_id);

  // Pack the strings
  std::vector<short int> temp; temp.reserve(bufsize);
  if (root_id == this->rank())
    {
  ...
  if (root_id != this->rank())
    {
      data.clear();
      std::vector<short int>::const_iterator iter = temp.begin();
      while (iter != temp.end())
        {
